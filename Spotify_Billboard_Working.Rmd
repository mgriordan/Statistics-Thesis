...
tracks$accuracy <- ifelse(tracks$billboard_flag == tracks$billboard_pred, 1, 0)
table(tracks$accuracy, tracks$decade)

logistic_accuracy_decade <- tracks %>% 
  group_by(decade) %>%
  summarise(avg_accuracy = mean(accuracy), num_artists = n_distinct(artists))

ggplot(logistic_accuracy_decade, aes(x = num_artists, y = avg_accuracy))+
  geom_point()


ggplot(logistic_accuracy_decade, aes(x = decade, y = avg_accuracy, fill = num_artists))+
  geom_bar(stat = "identity")
...
ggplot(tracks, aes(x = decade, fill = as.factor(billboard_flag)))+
